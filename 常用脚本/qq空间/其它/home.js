...
HOME_D_LAY = 50;
DEF_APPID = 0;
TOOLBAR_HTML = '<button type="button" class="change" title="更换QQ秀">换形象</button><button type="button" class="mirror" title="对此物品进行镜面翻转">镜面</button><button type="button" class="max_z_index" title="将此物品置于顶部">置顶</button><button type="button" class="min_z_index" title="将此物品置于底部">置底</button><button type="button" class="del" title="删除">删除</button><button type="button" class="remove_show" title="移出此人物">移出人物</button><button type="button" class="add_z_index" title="提高此物品层级">向上</button><button type="button" class="sub_z_index" title="降低此物品层级">向下</button><button type="button" class="show_btn" title="展开按钮">展开</button><button type="button" class="hide_btn" title="隐藏按钮">隐藏</button>';
HOME_D_USER_LAY = 900;
...
(function(){
    ...
    HomeBusController.SelectItem = function(oItem, bMult){
        ...
        debug("选中了ItemNO = " + oItem.m_iItemNo + "Item Id = " + oItem.m_oNode.id + "Layid = " + oItem.m_iUniCode);
        return 0;
    };
...
(function(){
    ...
    function MakeUserElement(oItem){
        ...
        if (oItem.m_iUin == GetQzoneLoginUin()) 
            oDivWord.innerHTML += '<button type="button" onclick="try{SendWords()};catch(e){}">修改</button>';
...
QSWORLD.TOOLBAR = function(oThis, sId, fnOnBtClick){
    ...
    if (!QSWORLD.TOOLBAR.MODE) {
        ...
        QSWORLD.TOOLBAR.BUTTON_HIDE = new QSWORLD.TOOLBAR.CommondButton("bt_hidden", "隐藏按钮", "隐藏", "hide_btn");
        QSWORLD.TOOLBAR.BUTTON_HIDE.htmlButton.onclick = function(){
        ...
        QSWORLD.TOOLBAR.BUTTON_ROLLLEFT = new QSWORLD.TOOLBAR.CommondButton("rotate_left", "对此物品进行逆时针旋转", "逆时针旋转", "rotate_left");
        QSWORLD.TOOLBAR.BUTTON_ROLLLEFT.htmlButton.onclick = function(){
            fnOnBtClick("rotate_left_event", oThis.m_arrSelectItem[0]);
        }
        QSWORLD.TOOLBAR.BUTTON_ROLLRIGHT = new QSWORLD.TOOLBAR.CommondButton("rotate_right", "对此物品进行顺时针旋转", "顺时针旋转", "rotate_right");
