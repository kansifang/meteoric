...
function frameworkErrorHandle() {
    if (ownermode) {
        if (window.g_ReadOnly) {
            ...
            QZONE.space.showTips("<span style=\"color:red;\">尊敬的用户，由于您的装扮方案没有获取成功，不建议进行自定义或装扮空间操作，以免原有装扮丢失。</span>");
...
function loadGFP(callback) {
    ...
    if (!flag) {
        ...
        t.load("/ac/qzone/v5/G/" + g_type + "/" + g_type + "G5" + ((g_V && g_V[g_type]) ? g_V[g_type] : "") + ".js", null, "utf-8");
    } else {
        if (typeof(callback) == "function") {
...
(function() {
    ...
    QZONE.space.setTheme = function(styleId) {
        ...
        if (st) {
            if (ua.ie) {
                st.imports[1].disabled = true;
                st.imports[2].disabled = true;
                var doAllLoad = function() {
                    doAllLoad._num++;
                    if (doAllLoad._num == 2) {
                ...
                doAllLoad._num = 0;
                var loadCss = function(id, url, onload) {
                    var cssLink = $(id) || document.createElement('link');
                    if (id) {
                        cssLink.id = id;
                    }
                    ...
                    QZFL.event.addEvent(cssLink, 'readystatechange',
                    function() {
                        if (cssLink && !(cssLink.readyState == "complete" || cssLink.readyState == "loaded")) {
                            return;
                        }
                        onload();
                        cssLink = null;
                ...
                loadCss('style_css_0', _lsUrl, doAllLoad);
                loadCss('style_css_2', _csUrl, doAllLoad);
            } else {
...
function fixView() {
    if (g_fullMode) {} else {
        ...
        if (window.screen && window.screen.availHeight && (window.screen.availHeight <= 600 || window.screen.availWidth <= 800)) {
            QZONE.dom.setStyle(document.body.parentNode, "overflow", "auto");
            QZONE.dom.setStyle(document.body, "overflow", "visible");
        }
    }
...
/*  |xGv00|e3c493382cf7e6ee24d0b44135fbd496 */
