...
!(function(){
	...
	GD.Browser = {
		...
		isIE : isIE,
        isIE6 : (isIE && parseInt(Sys.ie,10) == 6),
        isIE7 : (isIE && parseInt(Sys.ie,10) == 7),
		...
		isBorderBox : isIE && !(document.compatMode == "CSS1Compat")
    };	
	...
	GD.$extend(GD,{
		...
		unique : function(_arr) {
	        ...
	        while (index < _arr.length) {
	            if (flag[_arr[index]] == typeof(_arr[index])) {
	                _arr.splice(index, 1);
	                continue;
	            }
	            flag[_arr[index].toString()] = typeof(_arr[index]); 
				++index;
	        }
	        return _arr;
			/*var o = {};
		...
		currentStyle : function(element) {//返回DOM的margin偏移量
		...
		MM_preloadImages : function() {
		    ...
		    if(d.images){
		        ...
		        var i;
		        var j=d.MM_p.length;
		        var a=GD.MM_preloadImages.arguments;
	...
	GD.$extend(GD.Cookie,{
		...
		del : function(name,main,secure) {// 删除Cookie
		    var _c = new GD.Cookie(name,"null",-1,"",main,secure);
		    GD.Cookie.set(_c);
		}
	});
	/**
	...
	GD.$extend(GD,{
		...
		getClubHome : function(_cdomain) {//获取圈子的主页地址
			return "http://"+_cdomain+".q.9917.com/";
		},
		getTopicURL : function(cId,topicId) {//获取帖子查看地址
			return "http://q.9917.com/topicinfo/"+cId+"/"+GD.PathParse(topicId)+".html";
		},
		getNoteURL : function(_userId,_NId) {//获取日志查看地址
			return  GD.getUserHome(_userId) + "/blog/" + _NId;
		},
		getBookURL : function(bookId) {//获取书详细页地址
			return "http://book.9917.com/bookinfo/"+GD.PathParse(bookId)+".html";
		},
		...
		timedChunk : function(items, process, _this, callback) {//延时处理函数
	...
	GD.$extend(GD,{
		...
		getBeforTime : function(itemTime,_time) {//传入 2009-04-05 02:37:04 返回格式
			...
			newTime = GD.dynamicTime(_time,newTime.getTime(),4);
			return newTime;
		}
	});
	GD.$extend(GD,{
...
function testIllegalURL(str, isReplace){
	...
	var html = str.replace(/((https?|ftp|news):\/\/)?(([a-z0-9\-]+[\.。])+(aero|arpa|biz|com|coop|edu|gov|info|int|jobs|mobi|mil|museum|name|nato|net|org|pro|travel|[a-z]{2,3})|(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5]))(\/[a-z0-9_\-\.\%~]+)*(\/?([a-z0-9_\-\.\%]*)(\?[a-z0-9+_\-\.%=&]*)?)?(#[a-z][a-z0-9_]*)?/gim, function($0){
		if(!/\.(jpg|png|gif|bmp|mp3|wma|mp4|avi|mpeg|divx|rm|ra|asf|mov|cda|midi|wave|vqf|aif|aiff|au|ram|rmvb|swf|flv)$/gi.test($0)) {//非图片后缀时继续判断
			...
			}else if(/^((https?|ftp|news):\/\/)?([a-z0-9\-]+\.)*(9917\.com|xxsy\.net|hongxiu\.com|qidian\.com|zhulang\.com|jjwxc\.net|17k\.com|4yt\.net|readnovel\.com|163\.com|sohu\.com|sina\.com|qq\.com|126\.com|tom\.com|56\.com|tudou\.com|youku\.com|99972\.com)\/?/gim.test($0)){
				//匹配允许列表中的domain
			}else{
