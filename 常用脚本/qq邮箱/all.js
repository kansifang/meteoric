...
gnIEVer = /MSIE (\d+.\d+);
/i.test(gsAgent) && parseFloat(RegExp["$1"]);
  ...
  try
  {
    ...
    else
    {
      ...
      if (AH == ahy)
      {
        ...
        as.parentNode.insertBefore(
        mL.createContextualFragment(eq), ahy ? as: as.nextSibling);
      }
      else
      {
...
function addEvent(gx, ar, xY, mD)
{
  if (gx)
  {
    ...
    {
      gx[mD ? "removeEventListener": "addEventListener"](
      ar, xY, false);
    }
...
{
  as.outerHTML = [
  as.outerHTML.split(">")[0], ">", aGW(as), "</", as.tagName, ">"].join("");
}
E(GelTags(sW, as), aBb);
}
...
{
  return (bX || "").replace(
  new RegExp(regFilter(PD), bA), aGQ);
}
...
{
  return bX && bX.replace ? (bX.replace(/&lt;
  /g, "<").replace(/&gt;
  /g, ">").replace(/&amp;
  /g, "&").replace(/&quot;
  /g, "\"")) : bX;
...
{
  return bX && bX.replace ? (bX.replace(/&/g, "&amp;").replace(/\"/g, "&quot;
  ").replace(/</g, "&lt;
  ").replace(/>/g, "&gt;
  ")) : bX;
}
...
function getStrDispLen(bX)
{
  ...
  if (!Iz)
  {
    ...
    insertHTML(
    cR, "afterBegin", T(['<div id="$id$" ', 'style="width:1px;height:1px;overflow:auto;*overflow:hidden;white-space:nowrap;', 'position:absolute;left:0;top:0;">', '</div>']).replace(
    {
      id: abF
    }
    ));
...
return function(dv, dV, aol, aom)
{
  ...
  switch (hJ)
  {
    ...
    case "zoomMax":
    case "zoomMin":
    var ant = Math[hJ == "zoomMax" ? "min": "max"](
    akD, VC);
...
function getDomain(awU)
{
  ...
  awU ? 1 : 0][location.href.indexOf("foxmail.com") > -1 ? 0 : 1];
}
function getSid()
{
...
function reloadSignature(ps, ahw)
{
  ...
  createPanel(
  top, "signatureFrame", T(["/cgi-bin/getcomposedata?t=signature&fun=compose&sid=$sid$&qzonesign=$qzonesign$&r=$rand$"]).replace(
  {
    sid: getSid(),
    qzonesign: ahw ? "disp": "",
    rand: now()
...
function changeStyle(qS, nu)
{
  ...
  createPanel(
  top, zp, "", ["doRealChangeStyle( this, ", lv, ", ", Jm, ", \'", nu, "\', ", aeD, ")"].join(""), "display:none;", "cache", Ru);
}
...
msgBox.aAq = function()
{
  ...
  {
    insertHTML(
    top.document.body, "afterBegin", T(['<div id="msgBoxDIV" style="position:absolute;width:100%;', 'padding-top:4px;height:24px;top:$top$px;text-align:center;">', '<span></span>', '</div>']).replace(
    {
...
showProcess.aIB = function(ae)
{
  ...
  {
    insertHTML(
    top.document.body, "afterBegin", T(['<table id="$id$" cellspacing=0 cellpadding=0 border=0 ', 'style="position:absolute;top:$top$px;left:0;width:100%;display:none;">', '<tr><td align="center">', '<table cellspacing=0 cellpadding=0 border=0 class="autosave autosave_txt" style="height:20px;"><tr>', '<td style="width:2px;"></td>', '<td id="$id$_img" style="padding:0 0 0 5px;">', '<img src="$image_path$ico_loading.gif" style="width:16px;height:16px;">', '</td>', '<td id="$id$_plan" valign=center style="padding:0 0 0 5px;">', '<div style="font:1px;border:1px solid white;width:104px;text-align:left;">', '<div id="$id$_plan_bar" style="font:1px;background:#fff;height:8px;margin:1px 0;width:50%;"></div>', '</div>', '</td>', '<td id="$id$_plan_info" style="padding:0 0 0 5px;"></td>', '<td id="$id$_plan_rate" style="width:40px;text-align:right;padding:0;"></td>', '<td id="$id$_info" style="padding:0 0 0 5px;"></td>', '<td id="$id$_cancel" style="padding:0 0 0 5px;">', '[<a onclick="top.getMainWin().cancelDoSend();" nocheck="true" style="color:white;">取消</a>]', '</td>', '<td style="padding:0 0 0 5px;"></td>', '<td style="width:2px;"></td>', '</tr></table>', '</td></tr>', '</table>']).replace(
    {
      id: ae,
...
{
  return confirm(
  awr ? "你确认要清空此文件夹吗？": "你确认要删除此文件夹中的所有邮件吗？");
}
...
{
  var pZ = S(["folder_",
  (new String(bc)).toString().split("folder_").pop()].join(""), ao || getLeftWin());
  if (!pZ)
  {
    return 0;
  }
...
function RD(ag, cb, kE, cH, hq, Oc, awO, arO, Cn)
{
  ...
  {
    oP = ["", "&s=from_unread_list", "&s=from_star_list"][
    Oc != 1 && Oc != 2 ? 0 : Oc];
  }
  ak = Vn.replace(
  {
...
QMReadedItem.disp = function(Hl)
{
  ...
  {
    insertHTML(
    mB, "afterBegin", T(['<img src="$path$ico_grouplight.gif" class="showarrow"', ' title="这是您最近阅读的一封邮件" />']).replace(
    {
      path: getPath("image")
    }
    ));
...
modelDialog(
1, "举报并拒收选中邮件", ["<div style='padding:10px 10px 0 25px;text-align:left;'>", "<form id='frm_spamtype'>", "<div style='margin:3px 0 3px 3px'><b>请选择要举报的垃圾类型：</b></div>", Tj.replace(
{
value: (SI ? 11 : 8),
checked: "checked",
content: "其他邮件"
...
{
  var cm = [["<input type='button' class='qm_ico_quickup' title='隐藏' />", true], ["<input type='button' class='qm_ico_quickdown' title='显示更多操作' />", false]][
...
function ListMouseClick(ag)
{
  ...
  if (aT.className.indexOf("cir") == 0)
  {
    ...
    {
      Ib = GelTags("table", aT.parentNode.parentNode)[0].parentNode.onclick.toString().replace(/.*
      {
        /g, "").replace(/
      }
      .*/g, "").replace(/event/ig, "{shiftKey:true}");
      return eval(Ib);
    }
  }
...
{
  return this.adA(
  top.QMCache.Cq[this.DC].Vv, this.aEG) != 0;
};
...
function setMailListInfo(HV, DO)
{
  ...
  show(
  S("_uc", aY), isShow(Vj) && isShow(SQ));
  show(
  S("_ua", aY), isShow(Vj) || isShow(SQ));
  return hg;
}
...
function createPanel(ao, ae, aR, PE, Bx, ll, aqq)
{
  ...
  if (!gg)
  {
    ...
    insertHTML(
    cR, cR.getAttribute("loadcompleted") == "true" ? "beforeEnd": "afterBegin", T(['<iframe frameborder="0" scrolling="no" id="$id$" name="$id$" class="menu_base_if $className$" ', 'style="$style$" src="$url$" $event$ ></iframe>']).replace(
    {
      id: ae,
      className: ll,
      url: aR || "javascript:'';",
    ...
    if (!aR && ll && gg)
    {
      ...
      Pd.open();
      Pd.write(
      T('<html><body class="$className$">$content$</body></html>').replace(
      {
        className: ae,
        content: aqq
      }
...
function createMask(ao)
{
  ...
  {
    insertHTML(
    ao.document.body, "afterBegin", T(['<div id="$id$" class="editor_mask" style="display:none;"', ' onkeypress="return false;" onkeydown="return false;"', ' tabindex="0"></div>']).replace(
    {
      id: az
    }
    ));
...
function animation(as, eC, jy, Jl)
{
  ...
  var aAZ = [
  parseInt((eC.descLeft != null ? parseInt(eC.descLeft) - py[0] : 0) / jy), parseInt((eC.descTop != null ? parseInt(eC.descTop) - py[1] : 0) / jy), parseInt((eC.descWidth != null ? parseInt(eC.descWidth) - py[2] : 0) / jy), parseInt((eC.descHeight != null ? parseInt(eC.descHeight) - py[3] : 0) / jy)];
  var wH = 0;
var oV = function()
{
  if (++wH > jy)
...
function doPageError(ec, aR, EJ)
{
  try
  {
    ...
    {
      top.runUrlWithSid([
      abo, ec, "&line=", EJ, "&url=", encodeURIComponent(aR), "&func=", encodeURIComponent(Zs.replace(/[\r\n\t ]/ig, "").substr(
      0, 50))].join(""));
    }
...
aBJ: function(ag, atY)
{
  ...
  switch (cj)
  {
    ...
    {
      var aaP = S(
      OJ ? "quick_completelydel": "quick_del", getMainWin()),
...
aT.setAttribute("md", top.setTimeout(
function()
{
  aT.setAttribute("md", "0");
}
...
{
  addEvent(
  typeof(PH) == "string" ? S(PH, ao) : PH, "mousedonw",
function()
{
...
function startUpQQPlus(bA, awR)
{
  ...
  {
    loadJsFile(
    T(['$js_path$qqplus_kernel.js']).replace(cf));
  }
  if (!window.QQPlusUI)
  {
    loadJsFile(
    T(['$js_path$qqplus_ui.js']).replace(cf));
  }
  showQQPlusInfo("load",
  {
    title: "正在加载邮箱聊天..."
...
{
  goUrlMainFrm(
  T(['/cgi-bin/mail_list?sid=$sid$&s=search&folderid=all&page=0&subject=$subject$&sender=$sender$', '&receiver=$receiver$&searchmode=$searchmode$&advancesearch=0&loc=$loc$,$stat$']).replace(aC), false);
}
);
};
